...
def load_mapping():
    ...
    for row in rows[1:]:
        if not row:
            continue
        code = str(row[pos["Code"]]).strip() if "Code" in pos and pos["Code"] < len(row) and row[pos["Code"]] is not None else ""
        name = str(row[pos["Fund"]]).strip() if "Fund" in pos and pos["Fund"] < len(row) and row[pos["Fund"]] is not None else ""
        group = str(row[pos["Group"]]).strip() if "Group" in pos and pos["Group"] < len(row) and row[pos["Group"]] is not None else "기타"
        manager = str(row[pos["Manager"]]).strip() if "Manager" in pos and pos["Manager"] < len(row) and row[pos["Manager"]] is not None else "기타"
        team = str(row[pos["Team"]]).strip() if "Team" in pos and pos["Team"] < len(row) and row[pos["Team"]] is not None else "미분류"
...
function teamValues(){const all=[...new Set(DATA.funds.map(f=>f.team||'미분류').filter(Boolean))];return teamOrder.filter(t=>all.includes(t)).concat(all.filter(t=>!teamOrder.includes(t)).sort((a,b)=>a.localeCompare(b,'ko-KR')))}
function baseByTeam(){return DATA.funds.filter(f=>!selectedTeams.size||selectedTeams.has(f.team||'미분류'))}
